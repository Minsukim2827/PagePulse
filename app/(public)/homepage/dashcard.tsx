...
const DashCard: React.FC = () => {
  return (
    ...
    <div className="border-b-2 flex flex-col justify-center items-center p-4 h-auto relative gap-2 w-11/12 sm:w-full md:w-4/5 lg:w-7/10 mx-auto">
      <h1 className="text-black dark:text-white text-2xl md:text-3xl lg:text-6xl lg:pt-10 font-bold text-center">The next-gen book platform.</h1>
...
<button className="flex flex-row gap-2 items-center justify-center rounded-l px-16 py-3 text-l text:white bg-zinc-800 hover:bg-zinc-700">Sign Up <LogIn size={20}/></button>

</Link>
<Link href="/sign-in" passHref>
<button className="flex flex-row gap-2 items-center justify-center px-16 py-3 text-l text:white bg-zinc-800 hover:bg-zinc-700 ">Sign In <LogIn size={20}/></button>
        ...
        {cardContents.map((card, index) => (
          <div key={index} className="p-2 md:p-4 flex gap-4 md:gap-10 items-center">
            ...
            <div className="row-span-2 flex flex-col justify-between">
              ...
              <div className="text-md lg:text-xl font-bold p-2 text-zinc-400">{card.bottom}</div>
            </div>
          </div>
        ))}
      </div>
